...
def collect_fake_images():
    ...
    driver = webdriver.Chrome(
        executable_path=chromedriver_location, chrome_options=options
    )
    ...
    for attempt in tqdm(range(scraping_attempts)):
        try:
            driver.get(url)
            image = driver.find_element_by_id(element_id).get_attribute(
                attribute_to_retreive
            )
...
def deduplicate_fake_images():
    ...
    file_df = pd.DataFrame(
        [
            pd.Series(hash_list, name="hash"),
            pd.Series(filename_list, name="file"),
            pd.Series(size_list, name="size"),
        ]
    ).T
    ...
    print(
        f"Unique hash {num_unique_hash}, Not Unique hash {num_not_unique_hash}, Total unique hash {total_unique_hash}"
    )
    print(
        f"Unique size {num_unique_size}, Not Unique size {num_not_unique_size}, Total unique size {total_unique_size}"
    )
    ...
    # using hash
    hash_dupe_df = file_df.drop_duplicates(
        subset=["hash"], ignore_index=True, keep="first"
    ).reset_index(drop=True)
    remove_hash_df = file_df["file"][~file_df["file"].isin(hash_dupe_df["file"])]

    # using file size
    size_dupe_df = file_df.drop_duplicates(
        subset=["size"], ignore_index=True, keep="first"
    ).reset_index(drop=True)
    remove_size_df = file_df["file"][~file_df["file"].isin(size_dupe_df["file"])]
